...
import { initGoals, openGoalModal } from "./goals.js";
...
const navigateTo = (page) => {
  ...
  if (page === "portfolio") refreshPortfolioPage();
  if (page === "wealth") initWealth();
  if (page === "money") refreshMoneyPage();
...
const bindQuickActions = () => {
  document.getElementById("qa-add-txn")?.addEventListener("click", openAddModal);
  document.getElementById("btn-add-txn")?.addEventListener("click", openAddModal);
  document.getElementById("qa-add-inv")?.addEventListener("click", () => openInvModal(null));
  document.getElementById("btn-add-investment")?.addEventListener("click", () => openInvModal(null));
...
// PORTFOLIO PAGE
// ============================================================
const refreshPortfolioPage = async () => {
  ...
  const range = document.querySelector("#cashflow-filter .filter-btn.active")?.dataset.range || "30d";
  const { from, to } = dateRangeFor(range);
  const { income, expense, net } = calcCashFlow(txns, from, to);

  document.getElementById("cf-income").textContent = formatCurrency(income, "INR", true);
  document.getElementById("cf-expense").textContent = formatCurrency(expense, "INR", true);
  document.getElementById("cf-net").textContent = formatCurrency(net, "INR", true);

  const invs = getInvestmentsData();
  ...
  // Cashflow filter buttons
  document.querySelectorAll("#cashflow-filter .filter-btn").forEach(btn => {
    btn.onclick = () => {
      document.querySelectorAll("#cashflow-filter .filter-btn").forEach(b=>b.classList.remove("active"));
    ...
    if (matches.length === 0) {
      ...
      resultsBox.innerHTML = matches.map(t => `
        <div class="search-result-item">
          <span>${t.title}</span>
          <span style="margin-left:auto;font-weight:600">${formatCurrency(t.amount, t.currency||"INR")}</span>
        </div>`).join("");
    }
