...
// ============================================================
import { saveAsset, getAssets, deleteAsset, saveLiability, getLiabilities, deleteLiability } from "./database.js?v=20260705a";
import { formatCurrency, formatDate, todayISO, toast, openModal, closeModal } from "./utils.js?v=20260705a";
import { renderAssetAllocChart, renderNWTimeline } from "./charts.js?v=20260705a";
...
const openAssetModal = (asset) => {
  ...
  const body = `
    ...
    <div class="form-row"><label>Currency</label>
      <select id="a-currency" class="input">${["INR","SAR","USD","AED","GBP","EUR"].map(c=>`<option ${asset?.currency===c?"selected":""}>${c}</option>`).join("")}</select>
    </div>
...
const openLiabilityModal = (liab) => {
  ...
  const body = `
    ...
    <div class="form-row-inline">
      ...
      <div class="form-row"><label>Currency</label>
        <select id="l-currency" class="input">${["INR","SAR","USD","AED","GBP","EUR"].map(c=>`<option ${liab?.currency===c?"selected":""}>${c}</option>`).join("")}</select>
      </div>
    </div>
