...
// ============================================================
import { saveAsset, getAssets, deleteAsset, saveLiability, getLiabilities, deleteLiability } from "./database.js";
import { formatCurrency, formatDate, todayISO, toast, openModal, closeModal } from "./utils.js";
import { renderAssetAllocChart, renderNWTimeline } from "./charts.js";
...
export const renderWealthPage = () => {
  ...
  const sarEl = document.getElementById("wealth-nw-sar");
  if (sarEl) sarEl.textContent = "SAR " + (netWorth * 0.044).toFixed(2);
...
const renderLiabilitiesTable = () => {
  ...
  container.innerHTML = `
    <table class="asset-table">
      <thead><tr><th>Loan</th><th>Outstanding</th><th>Interest</th><th>EMI</th><th>Months Left</th><th></th></tr></thead>
      <tbody>
        ${_liabilities.map(l => `
          <tr>
            <td><strong>${l.name}</strong></td>
            <td class="negative">${formatCurrency(l.outstanding||0)}</td>
            <td>${l.interestRate||0}%</td>
            <td>${formatCurrency(l.emi||0)}</td>
            <td>${l.remainingMonths||0}</td>
            <td>
...
// ─── ASSET MODAL ──────────────────────────────────────────────
const ASSET_CATEGORIES = ["Real Estate","Vehicle","Gold/Jewelry","Cash","Bank Deposit","Other"];

const openAssetModal = (asset) => {
  const isEdit = !!asset;
  const body = `
    <div class="form-row"><label>Category</label>
      <select id="a-category" class="input">${ASSET_CATEGORIES.map(c=>`<option ${asset?.category===c?"selected":""}>${c}</option>`).join("")}</select>
    </div>
    ...
    <div class="form-row"><label>Currency</label>
      <select id="a-currency" class="input">${["INR","SAR","USD"].map(c=>`<option ${asset?.currency===c?"selected":""}>${c}</option>`).join("")}</select>
    </div>
...
const openLiabilityModal = (liab) => {
  ...
  const body = `
    ...
    <div class="form-row-inline">
      <div class="form-row"><label>Interest Rate (%)</label><input type="number" id="l-interest" class="input" value="${liab?.interestRate||""}" /></div>
      <div class="form-row"><label>Monthly EMI</label><input type="number" id="l-emi" class="input" value="${liab?.emi||""}" /></div>
    </div>
    <div class="form-row"><label>Remaining Months</label><input type="number" id="l-months" class="input" value="${liab?.remainingMonths||""}" /></div>`;
  ...
  document.getElementById("l-save").onclick = async () => {
    const data = {
      ...
      outstanding: parseFloat(document.getElementById("l-outstanding").value)||0,
      interestRate: parseFloat(document.getElementById("l-interest").value)||0,
      emi: parseFloat(document.getElementById("l-emi").value)||0,
      remainingMonths: parseInt(document.getElementById("l-months").value)||0,
    };
